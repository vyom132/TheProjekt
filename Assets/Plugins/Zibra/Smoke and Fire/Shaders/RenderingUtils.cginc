...
float DeltaT;
float StepScale;
float3 ContainerMinPoint;
float3 ContainerMaxPoint;
...
float BoxIntersectionOut(float3 ro, float3 rd, float3 minpos, float3 maxpos)
{
    float3 inverse_dir = 1.0 / rd;
    float3 tbot = inverse_dir * (minpos - ro);
    float3 ttop = inverse_dir * (maxpos - ro);
    float3 tmax = max(ttop, tbot);
    float2 traverse = min(tmax.xx, tmax.yz);
...
float TricubicSample(Texture3D<float> Texture, SamplerState TextureSampler, float3 texCoords, int3 TextureSize)
{
...
float SampleDensityTricubic(float3 pos, int3 TextureSize)
{
    return TricubicSample(Density, samplerDensity, WorldToUVW(pos), TextureSize);
}
...
{
    return Illumination.SampleLevel(samplerIllumination, WorldToUVW(pos), 0).rgb;
}

float4 SampleIlluminationDensityLinear(float3 pos)
{
...
float SampleShadowmap(float3 rayOrigin, float simulationScale)
{
    ...
    float3 normBoxPos = (boxPos - ContainerPosition) / ContainerScale + 0.5 / float3(ShadowGridSize);
...
float SampleShadowmapSmooth(float3 rayOrigin, float simulationScale)
{
    ...
    if (OpticalDensity < 5.0 || OpticalDensity > 1e-4)
        OpticalDensity = TricubicSample(Shadowmap, samplerShadowmap, normBoxPos + 0.5, ShadowGridSize);
...
float GetLightmapShadow(float3 samplingPos)
{
    ...
    float3 textureScale = 1.0 - 1.0 / LightGridSize;
...
bool TraceRay(float3 rayOrigin, float3 rayDirection, float tMax, inout RayProperties prop)
{
    ...
    float t = max(t0.y, 0.0) + DeltaT * DitherValues.x;
    tMax = min(tMax, t0.z);
    for (int i = 0; i < 256; i++)
    {
        t += DeltaT;
        if (t >= tMax || Sum(prop.absorption) < 1e-2) 
            break;

        float3 cpos = rayOrigin + t * rayDirection;
        float4 illDens = SampleIlluminationDensityLinear(cpos);
        float rho = illDens.a;
        if (rho < 1e-4) 
            continue;
        float3 col = illDens.rgb;
        IntegrateAbsorptionScattering(prop, StepScale * rho, col);
    }
    // Final step
    {
        float4 illDens = SampleIlluminationDensityLinear(rayOrigin + tMax * rayDirection);
        float weight = (tMax - (t - DeltaT)) / DeltaT;
        float rho = illDens.a;
        if (rho < 1e-4) 
            return true;
        float3 col = illDens.rgb;
        IntegrateAbsorptionScattering(prop, StepScale * rho * weight, col);
    }

    return true;
}

bool TraceRayNoBBCheck(float3 rayOrigin, float3 rayDirection, float tMax, inout RayProperties prop)
{
    const float boxOut = BoxIntersectionOut(rayOrigin, rayDirection, ContainerMinPoint, ContainerMaxPoint);
    tMax = min(tMax, boxOut);
    
    float t = DeltaT * DitherValues.x;
    for (int i = 0; i < 256; i++)
    {
        t += DeltaT;
        if (t >= min(boxOut, tMax) || Sum(prop.absorption) < 1e-2)
            break;
