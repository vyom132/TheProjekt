Shader "Hidden/ZibraEffects/SmokeAndFire/VisualizeSDF"
{
    SubShader
    {
        Pass
        {
            ...
            sampler2D SDFRenderSmoke;

#ifdef HDRP
            Texture2DArray<float> SmokeSDFVisualizationCameraDepth;
            SamplerState samplerSmokeSDFVisualizationCameraDepth;
#else
            ...
            {
                float4 sdfout = tex2D(SDFRenderSmoke, GetFlippedUV(uv * TextureScale));
...
#ifdef HDRP
                float sceneDepth = SmokeSDFVisualizationCameraDepth.Sample(samplerSmokeSDFVisualizationCameraDepth, float3(input.uv, 0));
#else
                float sceneDepth = _CameraDepthTexture.Sample(sampler_CameraDepthTexture, float3(input.uv, 0));
#endif
