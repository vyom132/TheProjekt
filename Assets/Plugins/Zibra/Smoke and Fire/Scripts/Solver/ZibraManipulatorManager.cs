...
namespace com.zibra.smoke_and_fire.Manipulators
{
    internal class ZibraManipulatorManager : MonoBehaviour
    {
        ...
        internal struct ManipulatorParam
        {
            ...
            public Int32 UnusedParameter1;
            public Int32 UnusedParameter2;
        ...
        {
            public Int32 EmitterIndexBegin;                      //Note: no texture emitters included
            ...
            public Int32 AnalyticEmitterIndexBegin;
            public Int32 AnalyticEmitterIndexEnd;

            public Int32 NeuralEmitterIndexBegin;
            public Int32 NeuralEmitterIndexEnd;
            public Int32 GroupEmitterIndexBegin;
            public Int32 GroupEmitterIndexEnd;
        }
        ...
        private Predicate<Manipulator> FindAnalyticSDF = delegate (Manipulator curManipulator) { return curManipulator.GetComponent<SDFObject>().GetSDFType() == SDFObject.SDFObjectType.Analytic; };
        private Predicate<Manipulator> FindNeuralSDF = delegate (Manipulator curManipulator) { return curManipulator.GetComponent<SDFObject>().GetSDFType() == SDFObject.SDFObjectType.Neural; };
        private Predicate<Manipulator> FindGroupSDF = delegate (Manipulator curManipulator) { return curManipulator.GetComponent<SDFObject>().GetSDFType() == SDFObject.SDFObjectType.Group; };
        /*
         * We need this ManipSDFPairs to properly calculate PrevPosition, PrevScale and PrevRotation.
         * Since the manipulators can be disabled and excluded from ManipulatorIndices the SDFObjectList can
         * be altered between the frames. We can't refer to the SDFObjectList by the index.
         * We allocate 2 here in order to avoid allocation of temporary dictionary on every frame. 
         */
        private Dictionary<int, SDFObjectParams> CurrentFrameManipSDFPairs = new();
        private Dictionary<int, SDFObjectParams> LastFrameManipSDFPairs = new();
        private void FlipFrameManipSDFPairs()
        {
            var temp = LastFrameManipSDFPairs;
            LastFrameManipSDFPairs = CurrentFrameManipSDFPairs;
            CurrentFrameManipSDFPairs = temp;
            CurrentFrameManipSDFPairs.Clear();
        }
        ...
        public void UpdateDynamic(ZibraSmokeAndFire parent, float deltaTime = 0.0f)
        {
            ...
            foreach (var manipulator in Manipulators)
            {
                ...
                manip.AdditionalData0 = simulationData.GetAdditionalData(0);
                manip.AdditionalData1 = simulationData.GetAdditionalData(1);
                manip.AdditionalData2 = simulationData.GetAdditionalData(2);
                ...
                {
                    var emitter = manipulator as ZibraSmokeAndFireEmitter;
                ...
                if(LastFrameManipSDFPairs.TryGetValue(manipulator.GetHashCode(), out var prevSDF))
                {
                    sdf.PrevPosition = prevSDF.Position;
                    sdf.PrevScale = prevSDF.Scale;
                    sdf.PrevRotation = prevSDF.Rotation;
                }
                else
                {
                ...
                ID++;
                if(manipulator.isActiveAndEnabled)
                {
                    CurrentFrameManipSDFPairs.Add(manipulator.GetHashCode(), sdf);
                }
            }
            FlipFrameManipSDFPairs();
        ...
        private void UpdateIndices()
        {
                         ...
                         typeof(Manipulator.ManipulatorType)))
            {
                ...
                Predicate<Manipulator> FindType = delegate (Manipulator curManipulator)
                {
                    return curManipulator.GetManipulatorType() == curManipulatorType &&
                           (curManipulator.isActiveAndEnabled || curManipulator is ZibraParticleEmitter || curManipulator is ZibraSmokeAndFireEmitter);
                };
            ...
            if (enabledForceFieldsCount > 0)
            {
                ...
                ffQuantity = forceFieldsList.FindAll(FindNeuralSDF).Count;
                ...
                ffQuantity = forceFieldsList.FindAll(FindGroupSDF).Count;
            ...
            int enabledEmittersCount = indices.EmitterIndexEnd - indices.EmitterIndexBegin;
            if (enabledEmittersCount > 0)
            {
                var emittersList = Manipulators.GetRange(indices.EmitterIndexBegin, enabledEmittersCount);

                int firstAnalytycEmitterIndex = emittersList.FindIndex(FindAnalyticSDF);
                int emittersQuantity = emittersList.FindAll(FindAnalyticSDF).Count;
                indices.AnalyticEmitterIndexBegin = indices.EmitterIndexBegin + firstAnalytycEmitterIndex;
                indices.AnalyticEmitterIndexEnd = indices.EmitterIndexBegin + firstAnalytycEmitterIndex + emittersQuantity;

                int firstNeuralEmitterIndex = emittersList.FindIndex(FindNeuralSDF);
                emittersQuantity = emittersList.FindAll(FindNeuralSDF).Count;
                indices.NeuralEmitterIndexBegin = indices.EmitterIndexBegin + firstNeuralEmitterIndex;
                indices.NeuralEmitterIndexEnd = indices.EmitterIndexBegin + firstNeuralEmitterIndex + emittersQuantity;

                int firstGroupEmitterIndex = emittersList.FindIndex(FindGroupSDF);
                emittersQuantity = emittersList.FindAll(FindGroupSDF).Count;
                indices.GroupEmitterIndexBegin = indices.EmitterIndexBegin + firstGroupEmitterIndex;
                indices.GroupEmitterIndexEnd = indices.EmitterIndexBegin + firstGroupEmitterIndex + emittersQuantity;
            }
            else
            {
                indices.AnalyticEmitterIndexBegin = 0;
                indices.AnalyticEmitterIndexEnd = 0;
                indices.NeuralEmitterIndexBegin = 0;
                indices.NeuralEmitterIndexEnd = 0;
                indices.GroupEmitterIndexBegin = 0;
                indices.GroupEmitterIndexEnd = 0;
            }
        }
